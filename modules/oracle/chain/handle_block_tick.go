...
func (o *ChainOracle) processChainRelay(
	...
	tx := makeTransaction(chainStatus.contractId, string(payloadJson), chainStatus.symbol, netId)
	...
	o.logger.Info("broadcast signature request",
		"symbol", chainStatus.symbol,
		"sessionID", sessionID,
		"txCid", txCid.String(),
	)
	...
	for signedWeight <= threshold {
		...
		case sigMsg := <-sigChan:
			...
			if err != nil {
				o.logger.Warn("failed to verify signature",
					"symbol", chainStatus.symbol,
			...
			if added {
				...
				signedWeight += weight
				o.logger.Info("collected signature",
					"symbol", chainStatus.symbol,
					"account", sigMsg.Account,
					"signedWeight", signedWeight,
					"threshold", threshold,
