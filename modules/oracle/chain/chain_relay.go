...
type ChainOracle struct {
	...
	//
	// review7 GV-H6: this map is written from the p2p message-handler goroutine
	// (witnessChainData) and read/deleted from the block-tick goroutine
	// (handleBlockTick), so every access MUST hold recentlyWitnessedMu — an
	// unsynchronized concurrent map write is a `fatal error: concurrent map
	// writes` that kills the process (uncatchable by recover).
