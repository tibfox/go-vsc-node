...
// signing party signs.
//
// review7 GV-H4: the ECDSA path must reject a non-32-byte digest rather than
// silently truncating it. hashToInt drops every byte past the 32nd, so two
// different messages sharing a 32-byte prefix would be signed to the SAME
// scalar — one threshold signature valid for both. We deliberately do NOT
// re-hash or domain-tag the digest here: the resulting ECDSA signature must
// verify on the destination chain (BTC/ETH) over the exact 32-byte sighash, and
// any transformation would make it unverifiable there. Cross-key replay across
// purposes is separately prevented by the per-contract key namespace
// (contractId-keyId) at the call site (execution-context.TssKeySign). A valid
// 32-byte digest is therefore signed verbatim — unchanged from before.
