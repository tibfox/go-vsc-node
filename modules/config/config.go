...
func (c *Config[T]) Init() error {
	...
	if err != nil {
		...
		// review7 GV-H7 / CRYP-13: the config can hold the BLS seed, Hive WIF
		// and libp2p private key. Run the world-bit-strip migration on the
		// existing-file path too — a node that only reads its config at startup
		// (never writes) would otherwise keep 0644 perms left by an earlier
		// build, leaving the keys world-readable. Group bits are preserved so
		// operators can still grant shared-group access deliberately.
