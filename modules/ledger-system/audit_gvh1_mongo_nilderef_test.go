...
// review7 GV-H1 — a transient Mongo fault in GetBalance panics the node.
//
...
// the consensus spend-check path crashed the process (recover() can't catch a
// nil-map/slice deref across the goroutine boundary in production). Silently
// treating the error as "no records" is equally unsafe — it would compute a
// balance from a partial read and fork this node from healthy peers. The fix
// is fail-stop: block-and-retry until the read succeeds, matching the
// authoritative UpdateBalances path (state_engine.go getLedgerRangeOrBlock).
//
// This test drives a DB that errors twice then recovers and asserts GetBalance
// returns the correct netted balance without panicking. Pre-fix it panics on
// the first error (recovered here into a clean t.Fatalf); post-fix it retries
// and returns the right number.
...
// flakyLedgerDb wraps MockLedgerDb and makes GetLedgerRange fail `failsLeft`
// times (returning the real Mongo failure shape: a nil slice pointer + error)
// before delegating to the real in-memory mock.
...
func TestAuditReview7_GVH1_GetBalanceFailStopsNotPanics(t *testing.T) {
	...
	_ = mock.StoreLedger(
		...
		ledgerDb.LedgerRecord{Id: "w", Owner: "hive:acct", Amount: -40000, Asset: "hbd", Type: "withdraw", BlockHeight: 20},
	)
	...
	require.Equal(t, int64(60000), bal, "balance must be correct after the DB recovers")
