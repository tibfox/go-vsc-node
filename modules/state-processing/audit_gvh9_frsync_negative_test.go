...
// review7 GV-H9 / C10-a — fr_sync accepts negative amounts and turns them into
// a phantom credit to the fractional-reserve virtual account.
//
// The handler computed `amt = -frSync.UnstakedAmount` whenever stake_amt <= 0.
// A gateway-signed (or replayed/malformed) fr_sync of {stake_amt:0,
// unstake_amt:-1000} therefore wrote a +1000 hbd_savings record to system FR
// with no L1 backing — the sign flipped a debit into a credit. The fix
// rejects any fr_sync whose amounts are negative.
...
func TestAuditReview7_GVH9_FrSyncRejectsNegativeAmounts(t *testing.T) {
	cases := []struct {
		name       string
		staked     int64
		unstaked   int64
